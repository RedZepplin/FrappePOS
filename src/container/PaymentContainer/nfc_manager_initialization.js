      ...
      FrappeFetch.createClient(returnUrl())
        .then(() => {
          ...
          return Client.postApi(
            "frappe_pos_sync.wallet_sync.check_customers_pin",
            {
    ...
    if (validUrl.isWebUri(returnUrl().url)) {
      FrappeFetch.createClient(returnUrl())
        .then(() => {
          const { Client } = FrappeFetch;
          return Client.postApi(
            "frappe_pos_sync.wallet_sync.validate_if_attendant_wallet_exists",
            {
    ...
    if (validUrl.isWebUri(returnUrl().url)) {
      FrappeFetch.createClient(returnUrl())
        .then(() => {
          const { Client } = FrappeFetch;
          return Client.postApi(
            "frappe_pos_sync.wallet_sync.validate_if_customer_wallet_exists",
            {
              wallet_card_number: tag.id,
  ...
  if (scanned_nfc) {
    ...
    if (validUrl.isWebUri(returnUrl().url)) {
      FrappeFetch.createClient(returnUrl())
        .then(() => {
          const { Client } = FrappeFetch;
          return Client.postApi("frappe_pos_sync.wallet_sync.validate_wallet", {
            wallet: scanned_nfc,
            receipt: json_object(defaultReceipt),
            device_id: deviceId,
          });
